import { r as __exportAll } from "./react-Cvh7Ylf6.js";
import { A as rotl, C as anumber, D as createHasher, E as concatBytes$2, F as rotlBH, I as rotlBL, L as rotlSH, M as swap32IfBE, N as toBytes$1, O as createView, P as u32, R as rotlSL, S as ahash, T as clean, _ as numberToHexUnpadded, a as bytesToHex$1, b as abytes, c as concatBytes$1, d as hexToBytes$1, f as inRange, g as numberToBytesLE, h as numberToBytesBE, i as bitMask, j as rotr, k as randomBytes, l as createHmacDrbg, m as memoized, n as abool, o as bytesToNumberBE, p as isBytes$1, r as bitLen, s as bytesToNumberLE, t as aInRange, u as ensureBytes, v as validateObject, w as aoutput, x as aexists, y as Hash, z as split } from "./utils-CC_BkgD7.js";
...
var InvalidAbiItemError = class extends BaseError$2 {
...
var InvalidAbiParameterError = class extends BaseError$2 {
	constructor({ param }) {
...
//#endregion
//#region node_modules/abitype/dist/esm/human-readable/parseAbiParameter.js
/**
...
function parseAbiParameter(param) {
	let abiParameter;
	if (typeof param === "string") abiParameter = parseAbiParameter$1(param, { modifiers });
	else {
		...
		for (let i = 0; i < length; i++) {
			...
			if (isStructSignature(signature)) continue;
			abiParameter = parseAbiParameter$1(signature, {
				modifiers,
				structs
			});
			break;
...
function parseAbiParameters(params) {
	...
	if (typeof params === "string") {
		...
		for (let i = 0; i < length; i++) abiParameters.push(parseAbiParameter$1(parameters[i], { modifiers }));
	} else {
		...
		for (let i = 0; i < length; i++) {
			...
			const length = parameters.length;
			for (let k = 0; k < length; k++) abiParameters.push(parseAbiParameter$1(parameters[k], {
				modifiers,
				structs
			}));
		}
...
var hash$1 = (value) => keccak256(toBytes(value));
...
var checksumAddressCache = /* @__PURE__ */ new LruMap$1(8192);
...
/** @internal */
var isAddressCache = /* @__PURE__ */ new LruMap$1(8192);
	...
	switch (abiParameterType) {
		...
		default:
			if (abiParameterType === "tuple" && "components" in abiParameter) return Object.values(abiParameter.components).every((component, index) => {
				return argType === "object" && isArgOfType$1(Object.values(arg)[index], component);
			});
...
function encodeEventTopics(parameters) {
	...
	if (eventName) {
		...
		if (!item) throw new AbiEventNotFoundError(eventName, { docsPath: docsPath$7 });
		abiItem = item;
	}
	if (abiItem.type !== "event") throw new AbiEventNotFoundError(void 0, { docsPath: docsPath$7 });
	let topics = [];
...
function prepareEncodeFunctionData(parameters) {
	...
	if (functionName) {
		...
		if (!item) throw new AbiFunctionNotFoundError(functionName, { docsPath: docsPath$6 });
		abiItem = item;
	}
	if (abiItem.type !== "function") throw new AbiFunctionNotFoundError(void 0, { docsPath: docsPath$6 });
	return {
		abi: [abiItem],
...
var NegativeOffsetError$1 = class extends BaseError {
	constructor({ offset }) {
		super(`Offset \`${offset}\` cannot be negative.`, { name: "NegativeOffsetError" });
	}
};
var PositionOutOfBoundsError$1 = class extends BaseError {
	constructor({ length, position }) {
		super(`Position \`${position}\` is out of bounds (\`0 < position < ${length}\`).`, { name: "PositionOutOfBoundsError" });
	}
};
var RecursiveReadLimitExceededError$1 = class extends BaseError {
...
var staticCursor$1 = {
	...
	assertReadLimit() {
		if (this.recursiveReadCount >= this.recursiveReadLimit) throw new RecursiveReadLimitExceededError$1({
			count: this.recursiveReadCount + 1,
			limit: this.recursiveReadLimit
		});
	},
	assertPosition(position) {
		if (position < 0 || position > this.bytes.length - 1) throw new PositionOutOfBoundsError$1({
			length: this.bytes.length,
			position
		});
	},
	decrementPosition(offset) {
		if (offset < 0) throw new NegativeOffsetError$1({ offset });
...
function decodeAbiParameters(params, data) {
	...
	for (let i = 0; i < params.length; ++i) {
		...
		const [data, consumed_] = decodeParameter$1(cursor, param, { staticPosition: 0 });
		consumed += consumed_;
		values.push(data);
	}
	return values;
...
function decodeParameter$1(cursor, param, { staticPosition }) {
	...
	if (param.type === "address") return decodeAddress$2(cursor);
...
function decodeArray$1(cursor, param, { length, staticPosition }) {
	if (!length) {
		const start = staticPosition + bytesToNumber(cursor.readBytes(sizeOfOffset$1));
...
var ContractFunctionExecutionError = class extends BaseError {
	constructor(cause, { abi, args, contractAddress, docsPath, functionName, sender }) {
		...
		const prettyArgs = prettyPrint({
			address: contractAddress && getContractAddress$1(contractAddress),
...
var WalletConnectSessionSettlementError = class WalletConnectSessionSettlementError extends ProviderRpcError$1 {
	constructor(cause) {
		super(cause, {
...
async function recoverPublicKey({ hash, signature }) {
	...
	const { secp256k1 } = await Promise.resolve().then(() => secp256k1_exports);
	return `0x${(() => {
...
//#endregion
//#region node_modules/viem/_esm/constants/number.js
...
*/
var sha256$2 = /* @__PURE__ */ createHasher(() => new SHA256());
//#endregion
...
/** @internal */
var supportsFillTransaction = /* @__PURE__ */ new LruMap$1(128);
/**
...
function decodeEventLog(parameters) {
	...
	if (!signature) throw new AbiEventSignatureEmptyTopicsError({ docsPath: docsPath$5 });
	const abiItem = abi.find((x) => x.type === "event" && signature === toEventSelector(formatAbiItem(x)));
	if (!(abiItem && "name" in abiItem) || abiItem.type !== "event") throw new AbiEventSignatureNotFoundError(signature, { docsPath: docsPath$5 });
...
function decodeFunctionResult(parameters) {
	...
	if (!abiItem.outputs) throw new AbiFunctionOutputsNotFoundError(abiItem.name, { docsPath: docsPath$4 });
...
function assertSize$1(bytes, size_) {
	if (size$3(bytes) > size_) throw new SizeOverflowError$2({
...
function assertStartOffset$1(value, start) {
	if (typeof start === "number" && start > 0 && start > size$3(value) - 1) throw new SliceOffsetOutOfBoundsError$2({
		offset: start,
		position: "start",
		size: size$3(value)
	});
...
function assertEndOffset$1(value, start, end) {
	if (typeof start === "number" && typeof end === "number" && size$3(value) !== end - start) throw new SliceOffsetOutOfBoundsError$2({
		offset: end,
		position: "end",
		size: size$3(value)
	});
...
function toBigInt$1(bytes, options = {}) {
	...
	return toBigInt(fromBytes$3(bytes, options), options);
}
/**
...
function toBoolean(bytes, options = {}) {
	...
	if (bytes_.length > 1 || bytes_[0] > 1) throw new InvalidBytesBooleanError$1(bytes_);
...
function toNumber$1(bytes, options = {}) {
	const { size } = options;
	if (typeof size !== "undefined") assertSize$1(bytes, size);
	return toNumber(fromBytes$3(bytes, options), options);
}
/**
...
var InvalidBytesTypeError = class extends BaseError$1 {
	constructor(value) {
		super(`Value \`${typeof value === "object" ? stringify$1(value) : value}\` of type \`${typeof value}\` is an invalid Bytes value.`, { metaMessages: ["Bytes values must be of type `Bytes`."] });
		Object.defineProperty(this, "name", {
			enumerable: true,
			configurable: true,
			writable: true,
...
function assert$5(value, options = {}) {
	...
	if (strict) {
		if (!/^0x[0-9a-fA-F]*$/.test(value)) throw new InvalidHexValueError$1(value);
	}
	if (!value.startsWith("0x")) throw new InvalidHexValueError$1(value);
}
/**
...
function encodeDeployData(parameters) {
	...
	if (!description) throw new AbiConstructorNotFoundError({ docsPath: docsPath$3 });
	if (!("inputs" in description)) throw new AbiConstructorParamsNotFoundError({ docsPath: docsPath$3 });
	if (!description.inputs || description.inputs.length === 0) throw new AbiConstructorParamsNotFoundError({ docsPath: docsPath$3 });
...
var supportsWalletNamespace$1 = new LruMap$1(128);
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/getContract.js
/**
...
function getContract({ abi, address, client: client_ }) {
	...
	if (hasPublicClient) {
		...
		if (hasEvent) {
			contract.createEventFilter = new Proxy({}, { get(_, eventName) {
				return (...parameters) => {
					const { args, options } = getEventParameters(parameters, abi.find((x) => x.type === "event" && x.name === eventName));
	...
	if (hasPublicClient || hasWalletClient) {
		if (hasWriteFunction) contract.estimateGas = new Proxy({}, { get(_, functionName) {
			return (...parameters) => {
				const { args, options } = getFunctionParameters(parameters);
				return getAction(publicClient ?? walletClient, estimateContractGas, "estimateContractGas")({
					abi,
					address,
					functionName,
					args,
...
function encodeErrorResult(parameters) {
	...
	if (errorName) {
		...
		if (!item) throw new AbiErrorNotFoundError(errorName, { docsPath: docsPath$2 });
		abiItem = item;
	}
	if (abiItem.type !== "error") throw new AbiErrorNotFoundError(void 0, { docsPath: docsPath$2 });
	const signature = toFunctionSelector(formatAbiItem(abiItem));
	let data = "0x";
	if (args && args.length > 0) {
		if (!abiItem.inputs) throw new AbiErrorInputsNotFoundError(abiItem.name, { docsPath: docsPath$2 });
...
//#endregion
//#region node_modules/viem/_esm/utils/data/isBytes.js
...
//#endregion
//#region node_modules/viem/_esm/utils/address/getContractAddress.js
...
function getCreate2Address(opts) {
	const from = toBytes(getAddress(opts.from));
	const salt = pad(isBytes(opts.salt) ? opts.salt : toBytes(opts.salt), { size: 32 });
	const bytecodeHash = (() => {
...
//#endregion
//#region node_modules/viem/_esm/utils/ccipTunnel.js
function ccipReadTunnel({ batchGateways, ccipRequest: ccipRequest$1 = ccipRequest }) {
	return { async request({ data, sender, urls }) {
		if (urls.includes("x-batch-gateway:true")) return ccipRequest$1({
			data,
			sender,
			urls: batchGateways
		});
			...
			if (failures[0]) {
				...
				try {
					...
					if (res.errorName === "HttpError") error = new HttpRequestError({
				...
				throw error ?? /* @__PURE__ */ new Error("An unknown error occurred.");
			}
			return responses[0];
		}
	} };
...
//#endregion
//#region node_modules/viem/_esm/utils/chain/extractChain.js
function extractChain({ chains, id }) {
	return chains.find((chain) => chain.id === id);
}
//#endregion
//#region node_modules/viem/_esm/utils/encoding/fromRlp.js
function fromRlp(value, to = "hex") {
	return fromRlpCursor(createCursor((() => {
		if (typeof value === "string") {
			if (value.length > 3 && value.length % 2 !== 0) throw new InvalidHexValueError(value);
			return hexToBytes(value);
		}
		return value;
	})(), { recursiveReadLimit: Number.POSITIVE_INFINITY }), to);
}
...
//#endregion
//#region node_modules/viem/_esm/utils/hash/isHash.js
...
//#endregion
//#region node_modules/viem/_esm/utils/hash/ripemd160.js
...
//#endregion
//#region node_modules/viem/_esm/utils/nonceManager.js
/**
...
var nonceManager = /* @__PURE__ */ createNonceManager({ source: jsonRpc() });
//#endregion
...
async function getWebSocketRpcClient(url, options = {}) {
	...
	return getSocketRpcClient({
		async getSocket({ onClose, onError, onOpen, onResponse }) {
			const WebSocket = await import("./native-DCJWOcFq.js").then((module) => module.WebSocket);
...
function keccak256$1(value, options = {}) {
	...
	const bytes = keccak_256(from$10(value));
...
function from$8(value) {
	const publicKey = (() => {
		...
		if (validate$4(value)) return fromBytes$2(value);
...
function toHex$1(publicKey, options = {}) {
	...
	return concat$1(includePrefix ? fromNumber(prefix, { size: 1 }) : "0x", fromNumber(x, { size: 32 }), typeof y === "bigint" ? fromNumber(y, { size: 32 }) : "0x");
}
/**
...
var InvalidError = class extends BaseError$1 {
	constructor({ publicKey }) {
		super(`Value \`${stringify$1(publicKey)}\` is not a valid public key.`, { metaMessages: [
...
var InvalidSerializedSizeError$1 = class extends BaseError$1 {
	constructor({ publicKey }) {
		super(`Value \`${publicKey}\` is an invalid public key size.`, { metaMessages: ["Expected: 33 bytes (compressed + prefix), 64 bytes (uncompressed) or 65 bytes (uncompressed + prefix).", `Received ${size$2(from$9(publicKey))} bytes.`] });
		Object.defineProperty(this, "name", {
			enumerable: true,
			configurable: true,
			writable: true,
...
function assert$3(value, options = {}) {
	const { strict = true } = options;
	if (!addressRegex.test(value)) throw new InvalidAddressError$1({
		address: value,
		cause: new InvalidInputError()
	});
	if (strict) {
		if (value.toLowerCase() === value) return;
		if (checksum(value) !== value) throw new InvalidAddressError$1({
			address: value,
...
function fromPublicKey(publicKey, options = {}) {
	return from$7(`0x${keccak256$1(`0x${toHex$1(publicKey).slice(4)}`).substring(26)}`, options);
}
/**
...
* ```
*/
var InvalidAddressError$1 = class extends BaseError$1 {
...
function decodeAddress(cursor, options = {}) {
	...
	return [wrap(fromBytes$3(slice$2(value, -20))), 32];
}
/** @internal */
...
function decodeBytes(cursor, param, { staticPosition }) {
	...
	if (!size) {
		...
		const data = cursor.readBytes(length);
		cursor.setPosition(staticPosition + 32);
		return [fromBytes$3(data), 32];
	}
	return [fromBytes$3(cursor.readBytes(Number.parseInt(size, 10), 32)), 32];
}
/** @internal */
...
function decodeString(cursor, { staticPosition }) {
	...
	const value = toString(trimLeft$1(cursor.readBytes(length, 32)));
	cursor.setPosition(staticPosition + 32);
	return [value, 32];
}
/** @internal */
...
function decode(parameters, data, options = {}) {
	...
	const bytes = typeof data === "string" ? fromHex$4(data) : data;
	...
	if (size$3(bytes) && size$3(bytes) < 32) throw new DataSizeTooSmallError({
		data: typeof data === "string" ? data : fromBytes$3(data),
		parameters,
		size: size$3(bytes)
	});
	let consumed = 0;
...
* Pass an array value.
*/
var InvalidArrayError$1 = class extends BaseError$1 {
	constructor(value) {
...
* @returns The RLP value.
*/
function fromHex$2(hex, options = {}) {
...
function numToSizedHex(num, size) {
	return bytesToHex$1(numberToBytesBE(num, size));
}
	...
	*/
	...
	function normPrivateKeyToScalar(key) {
		...
		if (lengths && typeof key !== "bigint") {
			if (isBytes$1(key)) key = bytesToHex$1(key);
...
function assert$2(signature, options = {}) {
	...
	if (signature.r < 0n || signature.r > maxUint256$1) throw new InvalidRError({ value: signature.r });
	if (signature.s < 0n || signature.s > maxUint256$1) throw new InvalidSError({ value: signature.s });
...
function fromBytes$1(signature) {
	return fromHex$1(fromBytes$3(signature));
}
/**
...
function from$4(signature) {
	const signature_ = (() => {
		if (typeof signature === "string") return fromHex$1(signature);
...
var InvalidSerializedSizeError = class extends BaseError$1 {
	constructor({ signature }) {
		super(`Value \`${signature}\` is an invalid signature size.`, { metaMessages: ["Expected: 64 bytes or 65 bytes.", `Received ${size$2(from$9(signature))} bytes.`] });
		Object.defineProperty(this, "name", {
			enumerable: true,
			configurable: true,
			writable: true,
...
function hash(authorization, options = {}) {
	...
	return keccak256$1(concat$1("0x05", fromHex$2(toTuple(presign ? {
		address: authorization.address,
...
*/
function recoverAddress$1(options) {
	return fromPublicKey(recoverPublicKey$1(options));
}
/**
...
*/
function recoverPublicKey$1(options) {
	const { payload, signature } = options;
	const { r, s, yParity } = signature;
	return from$8(new secp256k1.Signature(BigInt(r), BigInt(s)).addRecoveryBit(yParity).recoverPublicKey(from$9(payload).substring(2)));
}
//#endregion
//#region node_modules/ox/_esm/erc8010/SignatureErc8010.js
/**
* Magic bytes used to identify ERC-8010 wrapped signatures.
*/
var magicBytes$1 = "0x8010801080108010801080108010801080108010801080108010801080108010";
...
function unwrap(wrapped) {
	assert$1(wrapped);
	const suffixLength = toNumber(slice$1(wrapped, -64, -32));
...
function wrap$1(value) {
	...
	return concat$1(signature, suffix, fromNumber(size$2(suffix), { size: 32 }), magicBytes$1);
}
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/isErc8010Signature.js
/** Whether or not the signature is an ERC-8010 formatted signature. */
function isErc8010Signature(signature) {
	return validate$1(signature);
}
//#endregion
//#region node_modules/viem/_esm/utils/signature/parseErc6492Signature.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/parseErc8010Signature.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/recoverMessageAddress.js
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/recoverTypedDataAddress.js
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/serializeErc8010Signature.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/verifyHash.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/verifyMessage.js
/**
* Verify that a message was signed by the provided address.
*
* Note:  Only supports Externally Owned Accounts. Does not support Contract Accounts.
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/verifyTypedData.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/transaction/getSerializedTransactionType.js
function getSerializedTransactionType(serializedTransaction) {
	...
	if (serializedType !== "0x" && hexToNumber(serializedType) >= 192) return "legacy";
	throw new InvalidSerializedTransactionTypeError({ serializedType });
}
//#endregion
//#region node_modules/viem/_esm/utils/transaction/parseTransaction.js
...
//#endregion
//#region node_modules/viem/_esm/errors/unit.js
...
//#endregion
//#region node_modules/viem/_esm/utils/unit/parseGwei.js
/**
...
* @returns Whether or not the signature is valid. {@link VerifyHashReturnType}
*/
async function verifyHash$1(client, parameters) {
...
* @returns Whether or not the signature is valid. {@link VerifyMessageReturnType}
*/
async function verifyMessage$1(client, { address, message, factory, factoryData, signature, ...callRequest }) {
	const hash = hashMessage(message);
	return getAction(client, verifyHash$1, "verifyHash")({
		address,
		factory,
		factoryData,
		hash,
...
* @returns Whether or not the signature is valid. {@link VerifyTypedDataReturnType}
*/
async function verifyTypedData$1(client, parameters) {
...
function publicActions(client) {
	return {
		...
		verifyHash: (args) => verifyHash$1(client, args),
		verifyMessage: (args) => verifyMessage$1(client, args),
		verifySiweMessage: (args) => verifySiweMessage(client, args),
		verifyTypedData: (args) => verifyTypedData$1(client, args),
...
//#endregion
//#region node_modules/viem/_esm/actions/test/dropTransaction.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/dumpState.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/getAutomine.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/getTxpoolContent.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/getTxpoolStatus.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/impersonateAccount.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/increaseTime.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/inspectTxpool.js
/**
* Returns a summary of all the transactions currently pending for inclusion in the next block(s), as well as the ones that are being scheduled for future execution only.
*
...
//#endregion
//#region node_modules/viem/_esm/actions/test/loadState.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/mine.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/removeBlockTimestampInterval.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/reset.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/revert.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/sendUnsignedTransaction.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setAutomine.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setBalance.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setBlockGasLimit.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setBlockTimestampInterval.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setCode.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setCoinbase.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setIntervalMining.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setLoggingEnabled.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setMinGasPrice.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setNextBlockBaseFeePerGas.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setNextBlockTimestamp.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setNonce.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setRpcUrl.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/setStorageAt.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/snapshot.js
/**
...
//#endregion
//#region node_modules/viem/_esm/actions/test/stopImpersonatingAccount.js
/**
...
//#endregion
//#region node_modules/viem/_esm/clients/decorators/test.js
...
//#endregion
//#region node_modules/viem/_esm/clients/createTestClient.js
...
//#endregion
//#region node_modules/viem/_esm/types/eip1193.js
...
//#endregion
//#region node_modules/viem/_esm/utils/abi/decodeDeployData.js
...
function decodeDeployData(parameters) {
	...
	return {
		args: decodeAbiParameters(description.inputs, `0x${data.replace(bytecode, "")}`),
		bytecode
	};
}
//#endregion
//#region node_modules/viem/_esm/utils/blob/fromBlobs.js
...
//#endregion
//#region node_modules/viem/_esm/utils/blob/sidecarsToVersionedHashes.js
/**
...
function sidecarsToVersionedHashes(parameters) {
	...
	const hashes = [];
	for (const { commitment } of sidecars) hashes.push(commitmentToVersionedHash({
		commitment,
		to,
		version
	}));
...
//#endregion
//#region node_modules/viem/_esm/utils/ens/toCoinType.js
...
//#endregion
//#region node_modules/viem/_esm/utils/kzg/defineKzg.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/kzg/setupKzg.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/compactSignatureToSignature.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/parseCompactSignature.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/recoverTransactionAddress.js
async function recoverTransactionAddress(parameters) {
	...
	return await recoverAddress({
		hash: keccak256(serializeTransaction({
			...transaction,
			r: void 0,
			s: void 0,
			v: void 0,
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/serializeCompactSignature.js
/**
...
//#endregion
//#region node_modules/viem/_esm/utils/signature/signatureToCompactSignature.js
/**
